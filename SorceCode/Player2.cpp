...
Player2::Player2() {
    MHandle = MV1LoadModel("Assets/Dragon/Dragon.mv1");
    trans = VGet(320.0f, 100.0f, 550.f);
    rotation = VGet(0.f, 10.f, 0.f);
...
void Player2::Attack() {
    ...
    if (!key) {
        if (PAD.Buttons[8]) {
            ...
            //DrawBox(0, 400, 660, 700, Cr, TRUE);
            DrawExtendGraph(0, 400, 750, 800, GHandle5, TRUE);
            DrawString(280, 520, "ブレス（予定）", Cr2);
            DrawString(230, 570, "Ｘボタン", Cr2);
            //DrawFormatString(180, 460, Cr2, "残り使用回数%d", SPoint);
            //DrawExtendGraph(500, 550, 650, 650, GHandle3, TRUE);
            //DrawBox(1320, 400, 1980, 700, Cr, TRUE);
            DrawExtendGraph(1250, 400, 1980, 800, GHandle5, TRUE);
            DrawString(1500, 520, "攻撃強化（予定）", Cr2);
            ...
            //わざ２
            else if (PAD.Buttons[14] && SPoint > 0 && Level >= 3) {
                anim_id = 7;
                animatorPtr->Play(MHandle, anim_id);
                isAttack2 = true;
                command2 = true;
            ...
            //わざ３
            else if (PAD.Buttons[13] && Level >= 5) {
                anim_id = 3;
                animatorPtr->Play(MHandle, anim_id);
                notAttack = true;
                command2 = true;
...
void Player2::Status() {
    ...
    damage = HP / MAXHP * 100;
    damage2 = damage * 6;
    GHandle = LoadGraph("Assets/icon/Doragon_icon.png");
    ...
    // 読みこんだグラフィックを拡大描画
    DrawExtendGraph(-10, -15, 160, 150, GHandle, TRUE);// アイコンを描画
...
void Player2::AttackCollision() {
    ...
    vector<AttackCollider> attackCollider = {
        { "Bip001 L Hand" ,"Bip001 L Finger21" },
        { "Bip001 Head" ,"zuiba" },
    };

    for (int i = 0; i < attackCollider.size(); i++) {
        Player2Attack* attackPtr = new Player2Attack();
...
void Player2::transform_collider() {
    for (int i = 0; i < attackVec.size(); i++) {
        Player2Attack* attackPtr = (Player2Attack*)attackVec[i];
...
void Player2::SetupCollision(CollisionManager* managerPtr) {
...
void Player2::Update() {
    ...
    // カメラの位置はカメラの水平角度と垂直角度から算出
    {
        ...
        tempPosition2.x = cosParam * tempPosition1.x - sinParam * tempPosition1.z *1.5;
        tempPosition2.y = tempPosition1.y;
        tempPosition2.z = sinParam * tempPosition1.x + cosParam * tempPosition1.z*1.5;
    }
...
void Player2::OnCollisionHit(Collider* colliderPtr, GameObject* gobjPtr) {
    //ボスの攻撃を食らうとダメージを受ける
...
void Player2::SetupEffectManager(EffectManager* ptr) {
